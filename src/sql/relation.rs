...
struct VisitedQueryRelations<'a, T: QueryToRelationTranslator + Copy + Clone> {
    ...
    translator: T,
    aliases: Hierarchy<Vec<ast::Ident>>,
}
...
impl<'a, T: QueryToRelationTranslator + Copy + Clone> VisitedQueryRelations<'a, T> {
    ...
    fn try_from_join_constraint_with_columns(
        ...
        Ok(match join_constraint {
            ...
            ast::JoinConstraint::Using(idents) => {
                // the "Using (id)" condition is equivalent to "ON _LEFT_.id = _RIGHT_.id"
                Expr::and_iter(idents.into_iter().map(|id| {
                    Expr::eq(
                        Expr::Column(Identifier::from(vec![
                            LEFT_INPUT_NAME.to_string(),
                            id.value.to_string(),
                        ])),
                        Expr::Column(Identifier::from(vec![
                            RIGHT_INPUT_NAME.to_string(),
                            id.value.to_string(),
                        ])),
                    )
                }))
            ...
            ast::JoinConstraint::Natural => {
                // the NATURAL condition is equivalent to a "ON _LEFT_.col1 = _RIGHT_.col1 AND _LEFT_.col2 = _RIGHT_.col2" where col1, col2... are the columns present in both tables
                let tables = columns
                    .iter()
                    .map(|(k, _)| {
                        k.iter()
    ...
    fn try_from_join(
        ...
        let operator =
            self.try_from_join_operator_with_columns(&ast_join.join_operator, &all_columns)?;
        let join: Join = Relation::join()
            .operator(operator)
            .left(left_relation)
            .right(right_relation)
            .build();
        let join_columns: Hierarchy<Identifier> =
        ...
        let (relation, coalesced) = match &ast_join.join_operator {
            ...
            | ast::JoinOperator::FullOuter(ast::JoinConstraint::Using(v)) => {
                ...
                let to_be_coalesced: Vec<String> =
            ...
            match select_item {
                ast::SelectItem::UnnamedExpr(expr) => {
                    ...
                    let implicit_alias = match expr {
                        ast::Expr::Identifier(ident) => lower_case_unquoted_ident(ident),
                ...
                ast::SelectItem::Wildcard(_) => {
                    ...
                    let new_aliases: Hierarchy<String> = non_ambiguous_cols
                        .iter()
    ...
    fn try_from_select_items_selection_and_group_by(
        ...
        let split = if group_by.is_empty() {
            ...
            let group_by = group_by
                .clone()
                .into_iter()
                .fold(Split::Reduce(Reduce::default()), |s, expr| {
                    s.and(Split::Reduce(Split::group_by(expr)))
                });
            named_exprs
                .into_iter()
                .fold(group_by, |s, named_expr| s.and(named_expr.into()))
        };
        // Prepare the WHERE
...
impl<'a, T: QueryToRelationTranslator + Copy + Clone> Visitor<'a, Result<Arc<Relation>>>
    for TryIntoRelationVisitor<'a, T>
{
...
impl<'a, T: QueryToRelationTranslator + Copy + Clone> TryFrom<(QueryWithRelations<'a>, T)>
    for Relation
{
    type Error = Error;
...
impl<'a, T: QueryToRelationTranslator + Copy + Clone> TryFrom<(&'a ast::ObjectName, T)> for Identifier {
    type Error = Error;

    fn try_from(value: (&'a ast::ObjectName, T)) -> result::Result<Self, Self::Error> {
        let (object_name, translator) = value;
        let checked_identifiers: Result<Vec<String>> = object_name.0
            .iter()
            .map(|i| {
        ...
        Ok(checked_identifiers.iter().fold(Identifier::empty(), |acc, x|acc.with(x.to_string())))
    }
}
    ...
    for FullyQualifiedTableNames
{
    ...
    fn try_from(value: (&str, T)) -> result::Result<Self, Self::Error> {
        ...
        // Visit for conversion
        let tables_names: Result<Vec<Identifier>> = query_names.iter()
            ...
            .map(|object_name| {
                Identifier::try_from((object_name, translator))
            })
            .collect();
...
pub fn tables_prefix<T: QueryToRelationTranslator + Copy + Clone>(query: &str, translator: T) -> Result<Vec<String>> {
...
mod tests {
    ...
    use super::*;
    use crate::{
        builder::Ready, data_type::{DataType, DataTyped, Variant}, dialect_translation::{bigquery::BigQueryTranslator, mssql::MsSqlTranslator}, display::Dot, io::{postgresql, Database}, relation::schema::Schema
    };
    ...
    fn test_tables_prefix() {        
        ...
        let tables = tables_prefix(query_str, PostgreSqlTranslator).unwrap();
        assert_eq!(
            tables, vec!["my_db".to_string(), "my_other_db".to_string()]
        );
    }
    ...
    fn test_group_by_exprs() {
        ...
        let relation = Relation::try_from(QueryWithRelations::new(&query, &relations)).unwrap();
        relation.display_dot().unwrap();
        println!("relation = {relation}");
        assert_eq!(
            relation.data_type(),
    ...
    fn test_join_with_using() {
        ...
        if let DataType::Struct(s) = relation.data_type() {
            ...
            assert_eq!(
                s[1],
                Arc::new(DataType::optional(DataType::float_interval(20., 50.)))
            );
            assert_eq!(s[2], Arc::new(DataType::float()));
        }
        ...
        if let DataType::Struct(s) = relation.data_type() {
            assert_eq!(
                s[0],
                Arc::new(DataType::optional(DataType::integer_interval(-5, 10)))
            );
            assert_eq!(
                s[1],
                Arc::new(DataType::optional(DataType::float_interval(20., 50.)))
    ...
    fn test_join_with_natural() {
        ...
        if let DataType::Struct(s) = relation.data_type() {
            ...
            assert_eq!(
                s[1],
                Arc::new(DataType::optional(DataType::float_interval(-10., 100.)))
            );
            assert_eq!(
                s[2],
                Arc::new(DataType::optional(DataType::float_interval(20., 50.)))
