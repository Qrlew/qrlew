use crate::{
    ...
    hierarchy::Hierarchy,
    relation::{Join, Map, Reduce, Relation, Set, Table, Variant as _, transforms, Visitor},
    visitor::Acceptor,
    DataType,
    protected::PE_ID,
};
...
impl Reduce {
    /// TODO
    pub fn tau_thresholded_values(&self, epsilon: f64, delta: f64, sensitivity: f64) -> Result<Relation> {
        let tau = super::mechanisms::gaussian_tau(epsilon, delta, sensitivity);
        if self.input.schema().field(PE_ID).is_err() {
            return Err(Error::TauThresholdingError(format!("{PE_ID} column has not been found in the Relation")))
        }

        // compute distinct relation
        let group_by:Vec<String> = self.group_by.iter().cloned()
            .map(|x| {
                if let Expr::Column(name) = x {
                    Ok(name.to_string())
                } else {
                    Err(Error::TauThresholdingError("We support only `Expr::Column` in the group_by".into()))
                }
            })
            .collect::<Result<_>>()?;
        let group_by:Vec<&str> = group_by.iter()
            ...
            .collect();
        let aggregates = vec![
            (PE_DISTINCT_COUNT, aggregate::Aggregate::Count)
        ];
        let rel = self.input.as_ref().clone().distinct_aggregates(
            PE_ID,
            group_by.clone(),
            aggregates
        );
        let name_sigmas = vec![(PE_DISTINCT_COUNT, super::mechanisms::gaussian_noise(epsilon, delta, 1.))];
        let rel = rel.add_gaussian_noise(name_sigmas);

        // Returns Map with the right field names and with count(distinct) > tau
        let columns = vec![
            (PE_DISTINCT_COUNT, Some(tau.into()), None, vec![])
        ];
