...
/// A Visitor for the type  DataType
...
impl<'a, T: Clone, V: Visitor<'a, T>> visitor::Visitor<'a, DataType, T> for V {
    fn visit(&self, acceptor: &'a  DataType, dependencies: visitor::Visited<'a,  DataType, T>) -> T {
        match acceptor {
            ...
            _ => self.primitive(acceptor) // TODO ADD composite
        }
    }
}
...
/// TODO:A visitor to ..
#[derive(Clone, Debug)]
pub struct HierarchyVisitor;

impl HierarchyVisitor {
    fn composite<'a>(&self, fields: Vec<(String, Hierarchy<&'a DataType>)>) -> Hierarchy<&'a DataType> {
        fields.into_iter()
        .fold(
            Hierarchy::empty(),
            |acc, (s, h)|
            acc.chain(h.prepend(&[s.clone()])) // TODO : with ? remove chain ?
        )
    }
}

impl<'a> Visitor<'a, Hierarchy<&'a DataType>> for HierarchyVisitor {
    fn structured(&self, fields: Vec<(String, Hierarchy<&'a DataType>)>) -> Hierarchy<&'a DataType> {
        self.composite(fields)
    }

    fn union(&self, fields: Vec<(String, Hierarchy<&'a DataType>)>) -> Hierarchy<&'a DataType> {
        self.composite(fields)
    }

    fn primitive(&self, acceptor: &'a DataType) -> Hierarchy<&'a DataType> {
        Hierarchy::from([(Vec::<&str>::new(), acceptor)])
    }
}

impl DataType {
    pub fn hierarchy_2(&self) -> Hierarchy<&DataType> {
        self.accept(HierarchyVisitor)
    }
}
