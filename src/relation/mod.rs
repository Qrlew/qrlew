...
impl Join {
    ...
    fn schema(
        ...
        let transform_datatype_in_optional = match operator {
            JoinOperator::Inner(_) | JoinOperator::Cross=> false,
            _ => true,
        };
        let left_fields = left_names
            ...
            .map(|(name, field)| {
                Field::new(
                    name,
                    if transform_datatype_in_optional {DataType::optional(field.data_type())} else {field.data_type()},
                    if right_is_unique {
                        field.constraint()
                    } else {
                        None
