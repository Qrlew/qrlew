...
mod tests {
    ...
    fn test_display_values() {
        ...
        assert_eq!(
            query.to_string(),
            "WITH my_values (my_values) AS (SELECT * FROM (VALUES (3), (4)) AS my_values (my_values)), join_zs1x (field_utwr, field_uwjl) AS (SELECT * FROM my_values CROSS JOIN table AS table) SELECT * FROM join_zs1x".to_string()
        );
    }
}
